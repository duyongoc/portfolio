...
Which build hangs where, in what order, on which grid, is wall-layout.txt.
...
from portfolio import (ROOT, LAYOUT, GEN, CLIPDIR, SHARED, games, categories,
                       featured, slug, thumb_path, rgb, clips_on_disk)
...
def read_layout(path=LAYOUT):
    """wall-layout.txt -> a list of zone dicts, in file order.
...
def refresh_reference(all_games):
    ...
    if new != src:
        ...
        print('%-28s %d ids refreshed' % ('wall-layout.txt', len(all_games)))
...
def generate(zones, surfaces, ver, all_games):
    ...
    out = ['/* GENERATED by tools/wallsheet.py from wall-layout.txt.',
...
# them. Everything else the room fetches is either versioned already (the wall
# JPEGs, by WALL_SHEET_V) or a module, which the page names relative to itself.
STAMPED = ['portfolio-data.js', 'portfolio-shared.js', 'wall-layout.js']
PAGES = ['demos/room-3d.html', 'demos/portfolio-2d.html']
TAG = re.compile(r'<script src="(%s)(?:\?v=[0-9a-f]+)?"'
...
def stamps(gen):
    ...
    out = {}
    for name in STAMPED:
        body = (gen.encode('utf-8') if name == 'wall-layout.js'
...
def stamp_pages(ver):
    ...
    return {rel: TAG.sub(lambda m: '<script src="%s?v=%s"' % (m.group(1), ver[m.group(1)]),
...
def prepare():
    ...
    return dict(games=all_games, cats=cats, by_title=by_title, zones=zones,
                ...
                gen=gen, pages=stamp_pages(stamps(gen)))
...
def claims(st):
    ...
    if orphan:
        print('warn: the wall panel leads with the case files on the reasoning '
              'that all of them hang on zone %s, and %s no longer %s — fix the '
              'layout, FEATURED, or drop the claim from room-3d.html'
              % (lead['zone'], ', '.join(map(repr, orphan)),
                 'does' if len(orphan) == 1 else 'do'), file=sys.stderr)
    have = set(clips_on_disk(CLIPDIR))
...
def do_check(st):
    ...
    if ok:
        print('ok  %d zones, %d covers, %d props, %d clips, v=%s'
              % (len(st['zones']), len(st['seen']), len(st['surfaces']),
                 len(clips_on_disk(CLIPDIR)), st['ver']))
